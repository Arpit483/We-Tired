...
const Landing = () => {
  ...
  // BUG-09: relative URL — works on any host (Pi, staging, localhost)
  const API = '';

  // BUG-01 + BUG-04: poll /api/scan/result before calling stop to confirm
  ...
  const pollUntilFrames = async (maxAttempts = 5, intervalMs = 500) => {
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (d.frames_so_far > 0) return true;
      } catch { /* ignore network blips, retry */ }
      await new Promise(res => setTimeout(res, intervalMs));
    }
    return false; // no frames — stop anyway, scan_failed state will show
  };
  ...
  const startScan = async () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (count <= 0) {
        ...
        // BUG-01 fix: wait for frames before stopping
        (async () => {
          await pollUntilFrames(5, 500); // up to 2.5s extra grace period
          try {
    ...
    <main className="flex-grow flex flex-col lg:flex-row min-h-screen bg-[#141414]">
      ...
      <section className="w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r border-[#1a1a1a] p-6 md:p-12 flex flex-col justify-center relative overflow-hidden">
        ...
        <div className="border border-[#AAFF00] inline-block px-3 py-1 mb-8 self-start bg-[#141414] z-10">
          <span className="font-mono text-[10px] tracking-widest text-[#AAFF00] uppercase">
            ● RASPBERRY PI 4 · LD2410 DUAL RADAR · TCN+FFT
          </span>
        </div>
        ...
        <div className="mb-8 relative z-10">
          ...
          <p className="text-sm sm:text-base text-zinc-400 font-mono max-w-xl border-l-2 border-[#FF5C5C] pl-4 leading-relaxed">
            Non-invasive survivor detection for emergency disaster response. Dual LD2410D mmWave radar sensors detect micro-movement and respiratory patterns under rubble.
          </p>
        </div>
      ...
      <section className="w-full lg:w-1/2 p-6 md:p-12 flex flex-col items-center justify-center relative bg-[#141414]">
        <div className="w-full max-w-lg border border-[#1a1a1a] bg-[#0d0d0d] p-6 sm:p-8 flex flex-col gap-6 shadow-2xl">
          ...
          {/* ── SCANNING (polling / awaiting model result) ── */}
          {scanState === 'scanning' && (
            <div className="flex flex-col items-center gap-6 py-4">
              ...
              <div className="relative" style={{ width: 140, height: 140 }}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div style={{
                    width: 2, height: 60,
                    background: 'linear-gradient(to top, #00CFFF, transparent)',
          ...
          {scanState === 'result' && (() => {
            ...
            // BUG-08: distinct amber "failed" state when 0 frames collected
            if (failed) {
              return (
                <div className="flex flex-col items-center gap-6">
                  <div className="text-amber-400 font-mono text-[11px] tracking-widest uppercase">SCAN FAILED</div>
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-zinc-500 font-mono text-[11px] tracking-wider leading-relaxed px-2">
                      The radar sensor sent no frames during the scan window.<br />
                      Is <code className="text-amber-300">deep_optimized.py</code> running? Is the sensor connected?
                    </div>
                  </div>
                  <button
                    onClick={resetScan}
