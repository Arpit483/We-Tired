...
class Config:
    ...
    BAUD = 115200
    TIMEOUT = 0.5
    MODEL_PATH = os.path.join("model", "tcn_attention_vitalradar_v2.pt")
    SAMPLE_WINDOW = 64
...
def main():
    ...
    if HAS_TCN and not os.path.isfile(model_path):
        logger.critical(
            ...
            "  → System will run in FFT-only mode (reduced accuracy).",
            model_path,
        )
        tcn_engine = None
    elif HAS_TCN:
        try:
            ...
            logger.critical("[CRIT-02] TCN engine load failed: %s — FFT-only mode.", e)
            tcn_engine = None
    else:
        tcn_engine = None
