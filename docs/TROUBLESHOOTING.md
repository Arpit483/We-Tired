...
**Symptoms:** `Connection refused` in `deep.py`, `neo6m_runner.py`, or `firebase_bridge.py` logs
...
ps aux | grep python | grep -E "(app|deep|neo6m|firebase)"
...
# Test GPS endpoint
curl http://localhost:5050/api/gps/current
```
