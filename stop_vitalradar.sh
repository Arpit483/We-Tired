...
stop_service "GPS_Tracking" "python.*neo6m_runner.py"
stop_service "Firebase_Sync" "python.*firebase_bridge.py"

echo ""
echo "🔍 Checking for remaining processes..."
remaining=$(ps aux | grep -E "(deep_optimized|app\.py|neo6m_runner|firebase_bridge)" | grep -v grep)
if [ -n "$remaining" ]; then
