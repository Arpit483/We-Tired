...
# - GPS tracking (neo6m_runner.py)
# - Firebase sync (firebase_bridge.py)
...
pkill -f "python.*neo6m_runner.py" 2>/dev/null && echo "   → Stopped old GPS processes"
pkill -f "python.*firebase_bridge.py" 2>/dev/null && echo "   → Stopped old Firebase processes"
...
# 3. Start GPS Tracking (if neo6m_runner.py exists)
if [ -f "neo6m_runner.py" ]; then
    start_service "GPS_Tracking" "python3 neo6m_runner.py" "gps_tracking.log"
else
    echo "⚠️  GPS runner not found, skipping GPS service"
fi

# 4. Start Firebase Sync (if firebase_bridge.py exists)
if [ -f "firebase_bridge.py" ]; then
    start_service "Firebase_Sync" "python3 firebase_bridge.py" "firebase_sync.log"
else
    echo "⚠️  Firebase bridge not found, skipping Firebase service"
fi
...
ps aux | grep -E "(deep_optimized|app\.py|neo6m_runner|firebase_bridge)" | grep -v grep | while read line; do
    echo "   ✅ $line"
done
...
echo "   📄 GPS Tracking: gps_tracking.log"
echo "   📄 Firebase Sync: firebase_sync.log"
